...
if ($quantidade == 0) {
    ...
    $current_timestamp = time(); // Obtém o timestamp atual
    //Percorrendo todos os registros em aberto
    foreach ($resultado as $linha) {
        if ($linha->status === "aberto") {
            $timestampEnd = strtotime($linha->data_conclusao);

            if ($timestampEnd < $current_timestamp) {
                // Se a data de conclusão passou, adicione uma classe CSS para alterar a cor do card
                echo '<div class="card card-vencido" id="tarefa_' . $linha->id . '">';
                echo '<div class="card-body">';
                echo  '<div class="header-card d-flex">';
                echo    '<h5 class="card-title">'. $linha->titulo . '<span><i class="bi bi-exclamation-circle-fill"></i>Em atraso!</span></h5>';
            } else {
                // Caso contrário, use a classe padrão
                echo '<div class="card" id="tarefa_' . $linha->id . '">';
                echo '<div class="card-body">';
                echo  '<div class="header-card d-flex">';
                echo    '<h5 class="card-title">'. $linha->titulo . '</h5>';
            }
            // Se a atividade estiver aberta, exibe os detalhes
            ...
            echo '<button class="card-link btn btn-success alterarStatus" id="alterarStatus" data-atividade-id="'. $linha->id .'"><i class="bi bi-check"></i>Marcar como concluído</button>';
